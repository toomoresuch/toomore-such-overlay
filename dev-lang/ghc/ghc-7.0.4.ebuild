...
src_prepare() {
	...
	if use binary; then
		...
		if ! use ghcbootstrap; then
			case ${CHOST} in
				*-darwin* | *-solaris*)
                ...
                # cp "${FILESDIR}/iconv.txt" "${WORKDIR}"/ghc-bin-installer/ghc-7.0.4/libraries/base/base.buildinfo
        ...
        epatch "${FILESDIR}/configure-7.0.4.patch"
	fi
}
...
src_compile() {
	if ! use binary; then
        ...
        # cp "${FILESDIR}/ghc.mk" "${WORKDIR}"/ghc-7.0.4/libffi/
